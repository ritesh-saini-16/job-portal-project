  ...
  return (
    <div className='border p-6 shadow rounded '>
        ...
        <div className=' flex justify-between items-center mt-4 text-sm font-medium text-blue-600'>
            <button onClick={()=> {navigate(`/apply-job/${job._id}`)}} className='bg-blue-600 text-white px-4 py-1.5 rounded' > Apply Now</button>
            <button onClick={()=> {navigate(`/job-details/${job._id}`)}} className='bg-white border border-gray-200 px-4 py-1.5 rounded'> Learn More</button>
        </div>
    </div>
  )
}
